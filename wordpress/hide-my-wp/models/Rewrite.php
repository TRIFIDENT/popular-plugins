...
class HMWP_Models_Rewrite
{
    ...
    public function flushRewrites()
    {
        ...
        if (HMWP_Classes_Tools::isIIS() ) {
	        ...
	        if (!empty($this->_umrewrites)) {
		        foreach ( $this->_umrewrites as $rewrite ) {
			        $rewritecode .= 'Source: <strong>^' . str_replace(array('.css', '.js'), array('\.css', '\.js'), $rewrite['from']) . '</strong> Destination: <strong>' . $rewrite['to'] . "</strong> Redirect type: 301 Permanent;<br />";
		        }
	        }
	        ...
	        if (!empty($this->_rewrites) ) {
		        foreach ( $this->_rewrites as $rewrite ) {
			        if(PHP_VERSION_ID >= 70400 ){
				        $rewritecode .= 'Source: <strong>^/' . str_replace(array('.css', '.js'), array('\.css', '\.js'), $rewrite['from']) . '</strong> Destination: <strong>' . $rewrite['to'] . "</strong> Redirect type: Break;<br />";
