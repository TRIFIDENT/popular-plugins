	...
	$.fn.fbuilder = function(options){
		...
		if(!('cff_step' in $.validator.methods) && 'step' in $.validator.methods){
			$.validator.addMethod('cff_step', $.validator.methods['step']);
			$.validator.methods['step'] = function(value, element, param){
				if( $(element).closest('.cff-number-field,.cff-currency-field').length ) return true;
				return $.validator.methods['cff_step'].call(this, value, element, param);
			}
		}
		var items = [],
			fieldsIndex = {},
		...
		$.extend(fform.prototype,
			{
                ...
                after_show:function( id ){
                    ...
                    form.attr( 'data-evalequations', ('evalequations_delay' in this && this.evalequations_delay) ? 0 : this.evalequations )
						...
						.one ( 'showHideDepEvent', ( function( evalequations ){
							return function(){
								$(this).attr( 'data-evalequations', evalequations );
								if(evalequations) EVALEQUATIONS(this);
							};
						} )(this.evalequations) )
						.find( 'input,select' )
